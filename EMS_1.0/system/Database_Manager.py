...
def set_password(user_ID, new_password):
    ...
    passwords_file = Path(__file__).resolve().parent / 'passwords.csv'
...
class Employee:
    ...
    def issue_pay(self, payroll):
        ...
        if float(payroll) == 0:  # if the employee did not make anything, no payment is written to file
            ...
            paylog_file = Path(__file__).resolve().parent.parent / 'HoursReports' / 'paylog.txt'
...
objects.'''
    employees_file = Path(__file__).resolve().parent / 'employees.csv'
...
def main():
    load_employees()
    if Path(__file__).resolve().parent / 'employees.csv':
        pass
    else:
