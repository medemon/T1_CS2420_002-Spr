...
class Employee:
    ...
    def __init__(self, emp_id, first_name, last_name, address, city, state,\
                zipcode, classification, payment_method, route, accounting, phone, SSN,\
                DOB, start, end, title, dept):
    ...
    def issue_pay(self, payroll):
        ...
        if float(payroll) == 0: #if the employee did not make anything, no payment is written to file
            ...
            paylog_file = os.path.dirname(__file__) + '\\paylog.txt'
...
and the rate of commission. The rate is by default set to zero.'''
    ...
    def issue_payment(self):
        '''Stores the 1/24 of the total of the salary then adds up any receipts and multplies the
    ...
    with open(employees_file, 'r') as f:
        ...
        for line in data:
            if i == 0:
                ...
                e = Employee(id_num, first, last, addy, city, state, zipcody, classy, paymethod, routing,\
                accounting, phone, ssn, dob, start, end, title, dept) #instantiation
...
def process_timecards():
    ...
    timecards_file = os.path.dirname(__file__) + '\\timecards.csv'
...
def process_receipts():
    ...
    receipts_file = os.path.dirname(__file__) + '\\receipts.csv'
