...
def generate_title():
    ...
    return titles[random.randint(0, 2)]
...
def restart_system():
    ...
    with open(new_employees_file, 'w') as f:
        f.write("ID,first,last,Address,City,State,Zip,Classification,PayMethod,Salary,Hourly,Commission,Route,Account,Phone,SSN,DOB,Start_Date,End_Date,Title,Department\n")
        for line in lines:
...
def update_employee_file(emp_list):
    ...
    new_employees_file = os.path.dirname(__file__) + '\\employees_updated.csv'
    with open(new_employees_file, 'w') as f:
        f.write("ID,first,last,address,city,state,zip,classification,paymentmethod,salary,hourly,comission,routing,AccountingNum")
        f.write("phone,SSN,DOB,startDate,endDate,title,dept\n")
        for employee in emp_list:
            f.write(f"{employee.emp_id},{employee.first_name},{employee.last_name},{employee.address},{employee.city},{employee.state},")
            f.write(f"{employee.zipcode},{employee.classification},{employee.payment_method},{employee.route},{employee.accounting},{employee.phone},")
            f.write(f"{employee.SSN},{employee.DOB},{employee.start_date},{employee.end_date},{employee.title},{employee.dept}")
