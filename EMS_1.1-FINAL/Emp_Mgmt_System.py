...
class Window(Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.tk.call('source', os.path.dirname(__file__) + '\\UI\\Forest-ttk-theme-master\\forest-dark.tcl')
        ...
        self.iconbitmap(os.path.dirname(__file__) + '\\UI\\images\\window_icon.ico')
    ...
    def user_manual(self, page):
        ...
        img = Image.open(os.path.dirname(__file__) + '\\UI\\images\\' + page)
        user_image = ImageTk.PhotoImage(img)
        win.help_label = Label(win, width=790, height=790, image=user_image)
...
class Login_Screen(Frame):
    def __init__(self, parent, controller):
        ...
        # Help Button
        img = Image.open(os.path.dirname(__file__) + '\\UI\\images\\Help.png')
        ...
        img = Image.open(os.path.dirname(__file__) + '\\UI\\images\\Logo.png')
...
class Employee_Profile_Screen(Frame):
    def __init__(self, parent, controller):
        ...
        # Logout Button
        lo_img = Image.open(os.path.dirname(__file__) + '\\UI\\images\\logout.ico')
        logout_image = ImageTk.PhotoImage(lo_img)
        self.logout_button = Button(self, image=logout_image, command=lambda: controller.logout())
        self.logout_button.image = logout_image
        self.logout_button.place(x=960, y=650)
        ...
        self.password_button = Button(self, text="Change Password", width=15, bg="grey", font=("Tahoma", 10, "bold"),
        ...
        self.payroll_button = Button(self, text="Employee Pay Info", width=15, bg="grey", font=("Tahoma", 10, "bold"),
        ...
        # Report Button
        self.report_button = Button(self, text="Reports Screen", width=14, bg="grey", font=("Tahoma", 10, "bold"),
                                    command=lambda: controller.show_frame(Reports_Screen))
        self.report_button.place(x=150, y=540, anchor=CENTER)
...
class Reports_Screen(Frame):
    def __init__(self, parent, controller):
        ...
        self.save_button = Button(self, text="Issue Payroll", width=14, bg="grey", font=("Tahoma", 10, "bold"), command=lambda: save_txt())
        self.save_button.place(x=150, y=380, anchor=CENTER)

        # Save Pay log Button
        self.save_button = Button(self, text="Save Database Report", width=18, bg="grey", font=("Tahoma", 10, "bold"), command=lambda: save_csv())
        self.save_button.place(x=150, y=420, anchor=CENTER)
        ...
        def save_csv():
            ...
            original = os.path.dirname(__file__) + '\\system\\employees.csv'
            target = file.name
        ...
        def save_txt():
            data = [('CSV File', '*.csv')]
            file = asksaveasfile(filetypes=data, defaultextension='.csv')
            original = os.path.dirname(__file__) + '\\HoursReports\\paylogchart.csv'
            target = file.name
            shutil.copyfile(original, target)
...
class Employee_Payroll_Screen(Frame):
    def __init__(self, parent, controller):
        ...
        self.report_button = Button(self, text="Reports Screen", width=14, bg="grey", font=("Tahoma", 10, "bold"),
                                    command=lambda: controller.show_frame(Reports_Screen))
        self.report_button.place(x=150, y=500, anchor=CENTER)
    ...
    def receipt_popup(self, controller):
        ...
        receipt.note = StringVar()
        receipt.note_label = Label(receipt, textvariable=receipt.note, font=("Tahma", 15))
        if controller.admin.get() == True:
            a_state = "normal"
            receipt.note.set("")
        else:
            a_state = "disabled"
            receipt.note.set("Contact your supervisor if\nreceipts are wrong or missing.")
        add = Button(receipt, text='Add receipt', width=10, bg="grey", font=("Tahoma", 10, "bold"), state=a_state,
        ...
        for j in self.employee.classification.receipts:
            ...
            Button(receipt, text="Edit receipt", width=10, bg="grey", font=("Tahoma", 10, "bold"), state=a_state,
                                    command=lambda x=i-1: [self.edit_receipt_dialog(x, controller), receipt.destroy()]).place(x=180, y=y_loc)
            i += 1
            y_loc += 40
        receipt.note_label.place(x=20, y=y_loc)
    ...
    def hours_popup(self, controller):
        ...
        hours.note = StringVar()
        hours.note_label = Label(hours, textvariable=hours.note, font=("Tahma", 15))
        if controller.admin.get() == True:
            a_state = "normal"
            hours.note.set("")
        else:
            a_state = "disabled"
            hours.note.set("Contact your supervisor if\nhours are wrong or missing.")
        ...
        for j in self.employee.classification.timecard:
            ...
            i += 1
            y_loc += 40
        hours.note_label.place(x=20, y=y_loc)
...
class Search_Screen(Frame):
    def __init__(self, parent, controller):
        ...
        self.ID_entry = Entry(self, bd=1, textvariable=self.emp_ID, font=("Tahoma", 13))
        ...
        self.report_button = Button(self, text="Reports Screen", width=14, bg="grey", font=("Tahoma", 10, "bold"),
                                    command=lambda: controller.show_frame(Reports_Screen))
        self.report_button.place(x=165, y=450)
    ...
    def display_results(self, results_screen, controller, archived):
        ...
        get_last_name = self.last_name.get()
        ...
        for e in self.retrieved_employees:
...
def main():
    ...
    process_receipts()
    process_timecards()
    file_check = os.path.join('system', 'employees.csv')
...
testing = 0
# def test():
#     import testingFiles.Login_Test as lt
#     import testingFiles.Profiles_Test as pt
#     import testingFiles.PayScreen_Test as pst
#     import testingFiles.Search_Test as st
#     load_employees()
#     app = Window()
...
#     app.after(0, st.search_series(app.screens[Login_Screen], app.screens[Employee_Profile_Screen], app.screens[Search_Screen]))
