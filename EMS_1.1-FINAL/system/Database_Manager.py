...
import os # used to make abstract classes and methods
...
def set_password(user_ID, new_password):
    ...
    passwords_file = os.path.dirname(__file__) + '\\passwords.csv'
...
class Employee:
    ...
    def issue_pay(self, payroll):
        ...
        paylogchart_file = os.path.dirname(os.path.dirname(__file__)) + 'HoursReports\\paylogchart.csv'
        if float(payroll) == 0:  # if the employee did not make anything, no payment is written to file
            pass
        else:
            paylog_file = os.path.dirname(os.path.dirname(__file__)) + '\\HoursReports\\paylog.txt'
...
def process_timecards():
    ...
    timecards_file = os.path.dirname(os.path.dirname(__file__)) + '\\HoursReports\\timecards.csv'
...
def process_receipts():
    ...
    receipts_file = os.path.dirname(os.path.dirname(__file__)) + '\\HoursReports\\receipts.csv'
...
def run_payroll():
    ...
    r.close()
    file_check = os.path.dirname(os.path.dirname(__file__)) + '\\HoursReports\\' + PAY_LOGFILE
    if os.path.exists(file_check):  # pay_log_file is a global variable holding ‘payroll.txt’
